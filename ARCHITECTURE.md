...
- **Framework HTTP**: **Hono** para Node.js (Node 22/24) com `@hono/node-server` e `@hono/zod-openapi` selecionado formalmente via DEC-029 / ADR-010 (**005C: IMPLEMENTED / LOCAL + NEON STAGING INTEGRATION VALIDATED** — rotas, middleware de autenticação interna assimétrica tenant-scoped DEC-031/ADR-012, autorização multi-tenant e endpoints /v1 de Agent Studio validados contra Neon staging via TCP real. **Tenant Context Bootstrap**: **005D-B0: LOCAL IMPLEMENTATION / LOCAL INTEGRATION VALIDATED** via DEC-032 / ADR-013 — UserBootstrapAssertion, InternalBootstrapSigner, BootstrapAssertionVerifier, GET /v1/me/organizations, GET /v1/me/organizations/:orgSlug, BootstrapApiClient, UserOrganizationContextRepository e testes de integração local PostgreSQL; **Neon**: DATA/AUTHZ INTEGRATION VALIDATED em 005C / Neon Staging não acessado em 005D-B0; **API Deployment**: NOT DEPLOYED; **Production**: NOT PROVISIONED / UNTOUCHED; **Browser E2E**: NOT CLAIMED; **Slice 005D-B1**: NOT STARTED).
