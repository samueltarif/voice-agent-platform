...
6. **Fronteira de Confiança e Autenticação Interna de Serviços (DEC-029 / ADR-010)**:
   ...
   - **Status da Autenticação Interna de Serviços**: **005C: IMPLEMENTED / LOCAL + NEON STAGING INTEGRATION VALIDATED**. **Internal Service Auth**: STAGING CRYPTOGRAPHIC BOUNDARY VALIDATED (23 testes criptográficos unitários + 13 testes de integração em Neon Staging com par Ed25519 real, verificação de membership, RBAC dinâmico e cota). **Neon**: DATA/AUTHZ INTEGRATION VALIDATED. **API Deployment**: NOT DEPLOYED. **Production**: NOT PROVISIONED / UNTOUCHED. **Browser E2E**: NOT CLAIMED. **Slice 005D**: NOT STARTED.
