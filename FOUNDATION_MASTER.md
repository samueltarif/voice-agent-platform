...
8. **Transcrições & Áudios** (player integrado com gravação protegida por presigned URLs);
...
- **Monitoramento ao Vivo**: Transcrição, eventos, sentimentos e status em tempo real via WebSocket. Áudio ao vivo classificado como `STATUS: PLANNED / PROVIDER-DEPENDENT / NOT YET VALIDATED`.
- **Gravação de Chamadas**: Armazenamento em object storage com presigned URLs temporárias, isolamento multi-tenant e trilha de auditoria. Governança jurídica: `COMPLIANCE VERIFICATION REQUIRED BEFORE PRODUCTION`.
...
- Gravação de chamadas em Object Storage com presigned URLs temporárias e verificação regulatória pendente (DEC-023);
...
| 20 | Call Recording Compliance & Retenção | Análise jurídica de consentimento bilateral / LGPD | **COMPLIANCE VERIFICATION REQUIRED BEFORE PRODUCTION** |
