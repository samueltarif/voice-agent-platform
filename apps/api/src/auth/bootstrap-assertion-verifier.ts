...
export interface BootstrapVerifierConfig {
...
function validateProtectedHeader(token: string, knownKids: Set<string>): void {
  ...
  if (!knownKids.has(header.kid)) {
    throw new AuthenticationError(`Unknown key identifier '${header.kid}' in bootstrap assertion`);
  }
}
...
function validateTimestamps(
  ...
  if (exp - iat > BOOTSTRAP_ASSERTION_TTL_SECONDS) {
    throw new AuthenticationError(
      `Token lifetime (${exp - iat}s) exceeds maximum allowed TTL (${BOOTSTRAP_ASSERTION_TTL_SECONDS}s)`,
    );
  }
...
function validateClaims(payload: Record<string, unknown>): UserBootstrapAssertionClaims {
  const parseResult = userBootstrapAssertionClaimsSchema.safeParse(payload);
  if (!parseResult.success) {
    throw new AuthenticationError(
      `Invalid bootstrap assertion claims: ${parseResult.error.issues.map((i) => i.message).join('; ')}`,
    );
  }
  return parseResult.data;
}

export class BootstrapAssertionVerifier {
  ...
  constructor(config: BootstrapVerifierConfig) {
    if (!config.publicJwks || !Array.isArray(config.publicJwks.keys)) {
      throw new Error('BootstrapVerifier requires a valid JWKS object with a keys array');
    }

    this.knownKids = validateAndCollectKids(config.publicJwks.keys);
    this.expectedIssuer = config.expectedIssuer ?? DEFAULT_BOOTSTRAP_ISSUER;
    this.expectedAudience = config.expectedAudience ?? DEFAULT_BOOTSTRAP_AUDIENCE;
  ...
  async verify(token: string, nowSeconds?: number): Promise<UserBootstrapAssertionClaims> {
    if (!token || typeof token !== 'string') {
      throw new AuthenticationError('Missing or invalid bootstrap assertion token');
    }
    ...
    try {
      ...
      throw new AuthenticationError(`Bootstrap assertion verification failed: ${msg}`);
    }
